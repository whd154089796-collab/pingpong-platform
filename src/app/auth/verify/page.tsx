...
export default async function VerifyEmailPage({
  ...
  if (!token) {
    return (
      <div className="mx-auto max-w-2xl rounded-2xl border border-slate-700 bg-slate-900/75 p-8 text-center">
        ...
        <p className="mt-3 text-slate-300">缺少验证参数，请返回登录页重新发送验证邮件。</p>
        <Link href="/auth" className="mt-6 inline-block rounded-xl bg-cyan-500/20 px-4 py-2 text-cyan-100">返回登录页</Link>
      </div>
    )
  }
  ...
  return (
    <div className="mx-auto max-w-2xl rounded-2xl border border-slate-700 bg-slate-900/75 p-8 text-center">
      ...
      <Link href={result.error ? '/auth' : '/profile'} className="mt-6 inline-block rounded-xl bg-gradient-to-r from-cyan-500 to-blue-500 px-4 py-2 text-white">
        {result.error ? '去登录页' : '进入个人中心'}
      </Link>
    </div>
  )
}
